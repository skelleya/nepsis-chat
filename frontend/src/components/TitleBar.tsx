...
function SupportTitleBarButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      title="Support — open a ticket"
      aria-label="Support — open a ticket"
      className="flex h-full w-10 items-center justify-center text-app-muted transition-colors hover:bg-app-hover hover:text-app-text"
    >
 ...
 * Support + Update sit immediately left of the minimize button.
 */
    ...
    <>
      ...
      >
        ...
        {isMac ? (
          ...
          >
            <SupportTitleBarButton onClick={() => setSupportOpen(true)} />
