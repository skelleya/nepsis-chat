...
| **Black footer / bottom gap after login** | Nested `h-screen` inside a GSAP/`will-change-transform` shell left a compositor gap under the app (body `bg-app-darker` showed through). | **Fix**: `html, body, #root { height: 100% }`; app shell + MainLayout use `h-full`; fade-in uses `force3D: false` + `clearProps: 'transform'`. |
