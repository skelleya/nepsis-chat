...
| **Windows taskbar shows Electron icon** | Missing AppUserModelId and/or only PNG without multi-size ICO in the packaged exe. | **Fix**: `app.setAppUserModelId('com.nepsis.chat')`; ship `icon.ico` (build + runtime); `win.icon = build/icon.ico`. Rebuild installer (0.1.2+). |
| **Desktop shows pre-login / Guest** | Same LoginPage as web. | **Fix**: when `electronAPI.isElectron`, start on auth phase with Sign In/Sign Up only; block guest submit. |
...
| **Member profile opened as right panel; no Kick/Ban in profile** | `MemberProfilePanel` was a sibling column to the right of Members; moderation only lived on hover/context menu. Ban API/table did not exist. | **Fix**: Discord-style portal popout anchored left of the clicked member row; Kick/Ban with confirm for owner/admin; `POST .../members/:userId/ban` + `server_bans`; invite/join reject banned users. |
| **DM messages only appear after refresh** | `dm_messages` had RLS enabled but **no SELECT policies**, so Supabase Realtime (anon) never delivered inserts. REST (service role) still worked. | **Fix**: Migration `20250211000020_dm_rls_policies.sql` — SELECT/INSERT policies on DM tables. All-DM subscription also hydrates the open thread. |
...
| **Screen share not visible to peers** | `track.onmute` removed video during renegotiation; glare ignored remote offers; remote tracks lack `displaySurface`. | **Fix**: Don’t remove on mute; emit `screen-share` socket events; auto-watch remote share; better track heuristics. (Do **not** always-rollback glare — that broke join audio/video; use polite peer instead.) |
...
| Cannot create symbolic link (winCodeSign) | Windows symlink permissions on some local Windows setups | Prefer building on GitHub `windows-latest` with `signAndEditExecutable: true` (needed to embed the Nepsis icon). Local workaround: enable Developer Mode / symlink privilege, or keep packaging on CI. |
...
| Users can't see/hear each other in voice (each sees only themselves) | **Historical**: (1) ID-gated initiate with only `peer-joined` could leave nobody offering. (2) Answerer omitted local tracks. | **Partial fix then**: `room-peers` + always-initiate + add local tracks on answer. **Superseded**: always-initiate + always-rollback glare later broke all media — see next row. |
...
| **Members list shows "Unknown" for new/test accounts** | `GET /servers/:id/members` used `profile.display_name \|\| users.display_name \|\| 'Unknown'` and never selected `users.username`. Signup often left `display_name` null. | **Fix**: select `username`; fallback `profile → display_name → username → Unknown`. Guest/auth signup seeds `users.display_name` + personal `user_profiles` row. |
| **Other users can't see me in the call (presence shows In voice)** | `VoiceView` skipped anyone in `leftUserIds` when merging presence. Session-replace emits `peer-left` then rejoin — if rejoin was missed, the peer stayed hidden forever. | **Fix**: always merge `voiceUsersInChannel` into the participant circle list (presence wins). Mute badge no longer clipped by `overflow-hidden` on the avatar. |
| **Mute mic UI looks broken (red semi-circle on avatar)** | Mute badge was inside a `rounded-full overflow-hidden` avatar, so only a sliver of the red badge showed. | **Fix**: badge is a sibling of the circle (outside clip) with full red MicOff chip; control bar uses shared `MicIcon` / `MicOffIcon`. |
...
| **Server Unmute / Undeafen missing or menus stay stuck on Mute / Deafen** | The stack only implemented `mute-voice` / `deafen-voice` plus `admin-mute` / `admin-deafen`, so admins could force states on but not clear them. Some voice moderation menus also ignored the target user's live `remoteVoiceStates`. | Add `unmute-voice` and `undeafen-voice`, wire `admin-unmute` / `admin-undeafen` through signaling and `VoiceContext`, and render Mute vs Unmute / Deafen vs Undeafen from the current remote voice state. |
...
| **"High" mic processing sounds the same as Standard** | Browser supports echo/noise/AGC but does not expose `voiceIsolation` in `getSupportedConstraints()`. | Expected fallback: `high` becomes the same constraint set as `standard` on unsupported browsers. Check `navigator.mediaDevices.getSupportedConstraints().voiceIsolation` when debugging. |
| **`GET /api/users/:id/profiles` returns 403** | The route now requires `actorUserId === :id` and no longer serves profile rows without an owner-scoped query. | Call `/api/users/:id/profiles?actorUserId=:id`. Frontend helper: `api.getUserProfiles(userId, actorUserId)`. |
...
| **Backend crash loop: `io.sockets.sockets is not iterable`** — can't see anyone in server / voice join kills API | Single-session kick used `for (const [, s] of io.sockets.sockets)`, which is correct on the **root Server** (`server.sockets` = default namespace, `.sockets` = Map). Handlers receive the **`/voice` Namespace**, so `io.sockets` is already the Map; `.sockets.sockets` is undefined → `TypeError`, process exits, Railway restart loop. Members/presence/API briefly die with each join. | **Fix**: Iterate `io.sockets.values()` (Namespace Map). Guard if Map missing. File: `backend/src/socket/voice.js`. |
...
| **No Delete Account in User Settings** | Only guest logout called `DELETE /auth/guest/:userId`; registered users had no self-service purge; owned servers / emoji / invite FKs could block deletes. | **Fix**: `DELETE /api/auth/account/:userId` + shared `purgeUserAccount` (owned servers cascade, DMs, messages, invites, emojis, then user; `auth.admin.deleteUser` for linked accounts). My Account Danger Zone confirms with username. Files: `auth.js`, `api.ts`, `AppContext.tsx`, `UserSettingsModal.tsx`. |
| **Invite join stuck on “Joining…” / blank login after join** | `/invite/:code` is a separate route that unmounts `AppContent`. After join, `navigate('/')` remounts `AppContent` with `user` already set, so the `null → user` effect never ran and `showApp` stayed `false` (login shell forever). Refresh worked because session restore is `null → user`. “Log In to Join” also dropped the invite code. | **Fix**: Init `showApp`/`showLogin` from current `user`; also force `showApp` if remounted while authed. Persist `nepsis_pending_invite` for login handoff; after join set `joinServerId` + `nepsis_last_view: server`. Files: `App.tsx`, `InvitePage.tsx`. |
| **Camera / screen share shows black square** | (1) `getParticipantVideoStream` / `getScreenShareStream` created a new `MediaStream` every render → `srcObject` thrash. (2) Tiles used `bg-black` before frames arrived / while tracks were muted mid-renegotiation. (3) Camera tiles were small squares. | **Fix**: Cache derived streams by track id; `video.play()` + avatar placeholder until `loadeddata`/`playing`; auto-maximize camera to a large focus stage with filmstrip / dual stage / PiP. Files: `mediaTracks.ts`, `VoiceView.tsx`. |
| **DM reactions only local / not realtime** | Reactions lived in `DMView` component state; no `dm_message_reactions` table or API. | **Fix**: Migration `20250211000021_dm_reactions_reply.sql`; API + `subscribeToDMReactions`; `toggleDMReaction` in AppContext. |
| **No Call / Video Call in DM header** | Friends list had Call; DMView header only had overflow menu. | **Fix**: Phone + video buttons in DM header; `initiateCall(..., { video })`; expand active call from header / green bar. |
| **In-call bar couldn’t expand** | Compact green bar had no expand mode. | **Fix**: `callExpanded` + click-to-expand full panel (video stage when video call). |
| **Clicking a DM didn’t select reply** | No reply UI in DMs. | **Fix**: Click message / Reply → reply composer; `reply_to_id` on `dm_messages`. |
| **No Add Friend under All list** | Add Friend was only a top tab. | **Fix**: “+ Add a Friend” CTA under All friends → switches to Add tab. |
| **No download on chat/DM files** | Attachments were open-in-tab links only; DM URLs often rendered as plain text. | **Fix**: Shared `FileAttachment` with Download for channel attachments and DM media/file URLs. |
| **Can't hear peers / can't see remote camera after voice layout update** | (1) `addTrackToAllPeers` set `contentHint='detail'` on **all** video → cameras classified as screen → face tiles empty. (2) Focus/filmstrip remounted `RemoteAudio` → autoplay blocked → silence. (3) Lone 1080p camera treated as screen via width heuristic. | **Fix**: Only screen tracks set `contentHint`; use `knownScreenSharing` signal; persistent `RemoteAudio` at VoiceView root; drop `!muted` / width≥1280 camera→screen heuristics. Files: `webrtc.ts`, `mediaTracks.ts`, `RemoteAudio.tsx`, `VoiceView.tsx`. Railway logs (Node 18 deprecation) are unrelated. |
| **Guest + registered: no remote audio/video after join (post #38 still broken)** | Not guest-specific. `room-peers` + `peer-joined` both call `connectToPeer` (dual offer). Combined with “always rollback” glare (from screen-share fix), both peers accept each other’s offer then ignore answers in `stable` → no matched SDP → no `ontrack`. | **Fix**: One offerer via socket-id compare on join; restore polite/impolite glare (higher id yields). Flush `onRemoteStream` when `userId` arrives after tracks. File: `webrtc.ts`. |
...
| A second same-account tab hid/replaced voice presence | Each tab treated only its local WebRTC state as authoritative and an observer tab could overwrite `user_presence` with a null channel | Publish a per-account voice-owner heartbeat through localStorage/BroadcastChannel; observer tabs preserve presence, render self in the channel, and do not auto-join/kick the media owner |
| Soundboard MP3 rejected despite being valid | Electron/desktop uploads can label `.mp3` as `application/octet-stream`; the strict Multer MIME filter rejected it before metadata parsing | Permit octet-stream only for supported audio extensions, keep metadata validation as the content check, expose Multer errors as 400, and include AAC/FLAC in the picker |
...
| Soundboard sounds only visible to the uploader | `soundboard_sounds` was per-user; UI fetched `userId` only | Add `server_id`, list/upload by server membership, custom name on add/rename; migration `20260724023843_soundboard_server_scope.sql` |
...
| Appearance changed everywhere except the bottom user controls | `UserPanel` and EmojiPicker footer used fixed Discord gray/white values | Replace persistent shell backgrounds, text, status-ring borders, scrollbars, tooltips, update banner, and profile surfaces with `app-*` theme tokens |
| Channel rail could not collapse | Desktop channel wrapper was fixed at `w-72` and UserPanel had no compact mode | Add a persisted `w-72 ↔ w-14` desktop rail, compact channel icons/unread/voice state, expand/collapse controls, and stacked compact user controls |
| Cameras cramped or glitched when rails changed width | Voice layout used viewport breakpoints and fixed 220px grid minimums; video playback was not retried after container resize | Add inline-size container queries, dynamic 220/170/140px gallery columns, container-driven dual-stage stacking, and ResizeObserver playback retries without changing stream keys |
| Channel drag snapped back over voice rows / could not cross sections | Voice-user droppable targets stole channel collisions and channel-on-channel moves only handled the same category | Filter channel drag collisions, support direct cross-category channel drops, append order on category-header drops, and expose reorder/delete controls to admins as well as owners |
| Hard to drag users into other voice channels | Drop zone was only the tiny user list (`min-h-[2px]` when empty); user drags competed with channel/category sortables under `closestCenter` | Whole voice row is `voice-drop-*`; filter user collisions to voice drops + `pointerWithin`; enlarge empty pads + DragOverlay while dragging (0.2.7) |
| Patch notes listed newer versions under “Earlier releases” | `current = notes.find(installed) \|\| notes[0]` then everything else became “earlier” | Split by semver vs installed: Newer / Current·Installed / Earlier; keep “you’re on v…” subtitle (0.2.7) |
...
| **Right member sidebar flickers on hover and clicks do not open profiles** | `Section` was declared inside `MembersSidebar`; every presence refresh created a new component type and remounted every member button, potentially replacing it between pointer-down and click | Move stable `MemberSection` to module scope so presence/status renders update rows without unmounting their DOM nodes |
| **Settings dropdown flickers/closes while scrolling or testing mic** | Position updates restarted the GSAP enter tween; Voice & Video recreated device-option arrays and remounted menus on mic-meter renders | Animate enter once per open; stabilize options with `useMemo` + module constants; throttle mic-level state; keep `DeviceSelect` module-scoped + `memo`; isolate mic meter in `MicTestPanel` so RAF ticks never re-render sibling menus; `memo` on `SettingsDropdown`; ignore menu-internal scroll |
| **Cannot hear server voice while opening Friends → Add Friend** | WebRTC and the mic stayed connected, but `RemoteAudio` lived inside `VoiceView`; navigation unmounted every playback element | Mount remote audio sinks in `VoiceProvider` for the lifetime of the voice session; remove duplicate playback from `VoiceView` |
| **Cannot hear (or be heard) after opening Friends/DMs/chats** | Session sinks existed, but remote `TileVideo` still played unmuted audio in VoiceView; leaving that view dropped the only working playback path, and sinks inside the React tree could miss autoplay resume | Portal sinks to `document.body`; always mute camera/stage tiles (audio only via `RemoteAudio`); nudge `audio.play()` on main-view changes |
| **Windows desktop shortcut still shows Electron icon** | `signAndEditExecutable: false` skipped rcedit, so `Nepsis Chat.exe` kept Electron’s embedded icon; NSIS shortcuts use `exe,0` | Set `signAndEditExecutable: true` on CI Windows builds; set NSIS `installerIcon` / `uninstallerIcon` / `installerHeaderIcon` to `build/icon.ico`; reinstall after release |
| **Nepsis Chat appears multiple times in Task Manager** | Each shortcut click started a new Electron main process (close-to-tray kept the first alive) | `requestSingleInstanceLock()` + focus existing window on `second-instance`; keep `AppUserModelId` so helper processes group under one app |
| **Update shows “who should this be installed for?” wizard** | Assisted NSIS (`oneClick: false`) or non-silent quitAndInstall | `nsis.oneClick: true` + `quitAndInstall(true, true)` (`/S --updated`); in-app download/apply modals; Settings → Check for updates |
| **Update later did nothing / modal kept reappearing** | Dismiss flag was cleared whenever `updateDownloaded` stayed true | Superseded in 0.1.10: no auto-download / no ready modal; green badge only until the user starts the update |
...
| **Cannot hear anyone after per-user volume update** | Web Audio `MediaStreamSource` / `MediaElementSource` paths silenced remote WebRTC in Chromium/Electron | Revert to HTML `<audio>` playback; apply per-user volume with `audio.volume` (capped at 1.0); avoid overflow:hidden on the audio portal root |
| **Cannot hear anyone / they cannot hear me in voice (both ways)** | Answerer dropped trickle ICE that arrived before the peer PC existed (`handleIceCandidate` returned when `!peers.get(from)`). Offerer’s early candidates never applied → ICE failed → no media either direction | Buffer ICE per peer until PC exists **and** remote description is set; flush after `setRemoteDescription`. Also prepare answerer PC on `peer-joined`/`room-peers`. File: `webrtc.ts`. Configure TURN (`TURN_*`) when `/api/webrtc/ice` shows `hasTurn: false` for strict NAT |
| **Message feels delayed after hitting Enter** | `sendMessage` / `sendDMMessage` waited for the HTTP response before appending to state; DM also awaited before clearing the input | Optimistic temp messages + clear composer immediately; replace/dedupe when API or Realtime confirms |
| **No typing indicator in chat/DMs** | Backend `/chat` typing existed but frontend never joined rooms or emitted | `chatSocket` + `useChatTyping` in ChatView/DMView; `typing-stop` on idle/send |
| **Voice user menu stays open / sticks to faces** | Left-click both dismissed and reopened menus; overlays inside card stacking contexts missed click-away | Right-click only to open; portal + full-screen backdrop (`z` below menu) closes without hitting cards underneath |
| **Users can sign up without confirming email** | Supabase “confirm email” disabled | Keep signup open; show `EmailConfirmBanner` until `email_confirmed_at` is set (Resend supported) |
| **YouTube links only showed a static OG card** | Generic unfurl path | Detect YouTube IDs and render `YouTubeEmbed` (click-to-play iframe via youtube-nocookie) |
| **No cameras visible while reading chat in voice** | `VoiceView` unmounts on text/DM; session audio stayed but video UI did not | Mount `VoiceFloatingOverlay` under `VoiceProvider` (body portal); show when not on VoiceView |
...
| **Server mute/deafen had no reverse action** | Only force-on admin events existed | Add unmute/undeafen API + socket events; toggle menu labels from live voice state |
| **Camera rings / minimized status dots look cut off** | Ring/glow and status borders lived inside `overflow-hidden` media clips | Split decoration wrappers from clipped media; match expanded-row status-dot offsets |
| **Bios editable but not visible on profiles** | Settings already saved `user_profiles.bio`, but popouts omitted it and DMs lacked friend-resolved bio | Render About Me in `MemberProfilePanel`; enrich friend DM `other_user`; keep non-friend bios private |
