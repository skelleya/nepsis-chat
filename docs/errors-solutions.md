...
| **User Settings tabs flash when switching pages** | Content swapped on `activeTab` immediately while a short opacity/y fade ran on the new panel — looked like a hard cut/flash. | **Fix**: Directional slide — sidebar updates `activeTab` immediately; content exits horizontally by tab order, then `displayedTab` swaps, then new content enters from the opposite side. Clip with `overflow-x-hidden`; guard rapid clicks with `switching`. File: `UserSettingsModal.tsx`. |
