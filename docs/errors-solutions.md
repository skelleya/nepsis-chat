...
| **Login fields stayed open while joining** | Submit only flipped button text to “Joining…”; username/password block stayed fully visible. | **Fix**: On Continue / Sign In, GSAP collapses the credential fields (`height` + fade + slight rise) before auth; reopen on error or signup “check email”. File: `LoginPage.tsx`. |
...
| **User Settings tabs flash when switching pages** | Content swapped on `activeTab` immediately while a short opacity/y fade ran on the new panel — looked like a hard cut/flash. | **Fix (v1)**: Directional exit/enter slide. **Fix (v2)**: Horizontal strip of all pages — GSAP translates by panel width × index so My Account → Voice & Video slides through Profiles/Privacy/Appearance; sidebar highlight tracks intermediates; duration scales with distance. File: `UserSettingsModal.tsx`. |
