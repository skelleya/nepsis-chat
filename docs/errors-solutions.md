...
| **User Settings tabs flash when switching pages** | Content swapped on `activeTab` immediately while a short opacity/y fade ran on the new panel — looked like a hard cut/flash. | **Fix**: Horizontal page strip glides in one continuous motion past intermediates; sidebar highlight jumps to the destination (not stepped per page). File: `UserSettingsModal.tsx`. |
