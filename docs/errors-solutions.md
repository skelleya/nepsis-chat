...
| **User Settings scrollbar overlaps close (X)** | Close button was `absolute` on the modal; content `overflow-y-auto` drew its scrollbar under the same top-right corner. | **Fix**: Put the X in a dedicated `w-12` right rail beside the scroll pane so the scrollbar sits left of the button. File: `UserSettingsModal.tsx`. |
