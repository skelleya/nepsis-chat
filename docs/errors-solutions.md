...
| **Update later did nothing / modal kept reappearing** | Dismiss flag was cleared whenever `updateDownloaded` stayed true | Only re-open the modal when a new version key arrives; keep a top-right download badge to apply later |
...
| **NSIS asks “install for all users or just me” on update** | `quitAndInstall(false, true)` ran a non-silent assisted installer, which still shows install-mode pages | Auto-download updates, then `quitAndInstall(true, true)` so NSIS reuses the existing InstallLocation silently |
