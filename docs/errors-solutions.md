...
| **Black footer / bottom gap after login** | Nested `h-screen` inside a GSAP/`will-change-transform` shell left a compositor gap under the app (body `bg-app-darker` showed through). | **Fix**: `html, body, #root { height: 100% }`; app shell uses `fixed inset-0 overflow-hidden`; MainLayout `h-full`; fade-in uses `force3D: false` + `clearProps: 'transform'`. |
| **User Settings box jumps when changing tabs** | Modal used `max-h` only, so shorter tabs (Appearance, etc.) shrank the dialog. | **Fix**: Lock modal to `h-[min(640px,90vh)]`; content pane scrolls; GSAP fades tab content without resizing the shell. |
