...
| **Download 404 — GitHub releases page shows "There aren't any releases here"** | No **published** GitHub Release yet (drafts don't count for `/releases/latest`). | Run **Actions → Desktop Release** (builds Win+Mac and publishes), or `npm run release:win` / `release:mac` with `GH_TOKEN`. Confirm the release is not left as draft. |
