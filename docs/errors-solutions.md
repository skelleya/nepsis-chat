...
| **Cannot hear anyone after per-user volume update** | Pure `MediaStreamSource` Web Audio path failed to play remote WebRTC audio reliably | Play via HTML `<audio>` again; route through `createMediaElementSource` + `GainNode` for 0–200% |
| **Voice user menu stays open after click-away** | Backdrop lived inside transformed/scroll card parents so `fixed` overlay missed clicks | Portal menu to `document.body`; close on document `pointerdown` / Escape |
