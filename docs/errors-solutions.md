...
| **Settings dropdown flickers/closes while scrolling or testing mic** | Position updates restarted the GSAP enter tween; the Voice & Video tab declared its device selector inside the component, so mic-meter renders remounted open menus | Reposition without replaying enter animation, ignore menu-internal scroll, defer outside-click registration, and keep `DeviceSelect` at module scope |
