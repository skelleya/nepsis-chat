...
| **Cannot hear anyone / they cannot hear me in voice (both ways)** | (1) Answerer dropped trickle ICE before PC existed. (2) Production ICE was STUN-only (`hasTurn: false`) so strict-NAT peers never got a media path | Buffer ICE until remote description; prepare answerer PC on join. Default free Open Relay TURN from `/api/webrtc/ice` when `TURN_URLS` unset; optional ICE restart once. Prefer custom `TURN_*` for scale |
