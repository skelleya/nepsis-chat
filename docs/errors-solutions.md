...
| **Friends page had extra hairline strokes** | Header + tabs used `border-b border-app-dark/50` (and `/80`) as Discord-style section rules. | **Fix**: Removed those borders so the Friends header/tabs read as one open surface. File: `FriendsPage.tsx`. |
