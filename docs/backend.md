...
| dm_conversations | id, created_at, is_group, name, created_by, updated_at — 1:1 and group DM metadata; group fields added by `20260724004517_group_direct_messages.sql` |
...
Security note: these endpoints enforce conversation membership and validate target user IDs. Like the pre-existing REST API, identity is still supplied as `userId` rather than bound by auth middleware; replacing that legacy trust model requires a separate API-wide authentication migration.
