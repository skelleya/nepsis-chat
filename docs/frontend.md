...
| VoiceView | Voice participants, join/leave; soundboard. **Discord screenshare:** click LIVE / tile to watch (not auto-forced); resizable stage while watching; LIVE in channel list. Uses `react-resizable-panels`. **Admin:** right-click participant for Mute or Disconnect. |
