...
| ChannelList | Text + voice channels in a **Nepsis rail** (rounded rows, chat/wave icons, orange selected accent — not Discord `#` / ALL CAPS). Server banner above header when set; unread/mention highlights on text. **Friends view:** Friends header + DMs only. **Owner:** drag categories/channels; Rename/Delete menus. **Admin/owner:** each voice user has a side **⋯** menu — Move to… (other rooms), Server Mute, Disconnect; drag-drop still works. Live badge / Watch Live for screen share. |
