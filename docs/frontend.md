...
| UserSettingsModal | User Settings modal: fixed size (`h-[min(640px,90vh)]`) so tab changes don’t resize; GSAP open/close; **sidebar** uses a shared accent pill that slides between nav buttons (compact 15px labels, Log Out pinned at bottom); **content** exits upward + fades, then the next page fades/slides up; close (X) sits in a right rail so the content scrollbar never overlaps it. My Account / Profiles / Privacy as before. **Appearance:** Coming soon. **Voice & Video:** mic/speaker/camera device pick, output volume, echo/noise/AGC, mic test — applied on next voice join/call. **Notifications:** message/DM/call/voice sound toggles + browser call/DM alerts — localStorage `nepsis_user_prefs`. Shared **SettingsDropdown** (portal menu, GSAP open/close) and **SettingsToggle** (GSAP knob + track) used on Privacy / Voice / Notifications / Profiles. |
