...
| UserSettingsModal | User Settings modal: fixed size (`h-[min(640px,90vh)]`) so tab changes don’t resize; GSAP open/close + **horizontal page strip** — content glides past intermediate pages in one continuous slide; sidebar highlight jumps straight to the selected tab (does not step through each page). My Account: display name, username, avatar/banner. **Profiles:** Personal/Work identity (name/avatar/banner), active server profile, per-friend visibility (Personal / Work / Both). **Privacy & Safety:** DMs, calls, friend requests, voice-channel visibility, online status, speaking indicator. |
...
| LoginPage | Guest (username only), Sign In (email or username + password), Sign Up (email + password). **GSAP:** soft page/card enter; sliding accent pill on tabs; form panel slides with tab direction; credential fields collapse/close on Continue/Sign In (reopen on error); logo coin uses `quickSetter(..., 'rotationY', 'deg')` so horizontal cursor swipe spins the 3D coin with inertia + face settle |
