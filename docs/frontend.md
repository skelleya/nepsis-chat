...
| UserSettingsModal | User Settings modal: fixed size (`h-[min(640px,90vh)]`) so tab changes don’t resize; GSAP open/close; **sidebar** uses a shared accent pill that slides between nav buttons; **content** exits upward + fades, then the next page fades/slides up. My Account: display name, username, avatar/banner. **Profiles:** Personal/Work identity (name/avatar/banner), active server profile, per-friend visibility (Personal / Work / Both). **Privacy & Safety:** DMs, calls, friend requests, voice-channel visibility, online status, speaking indicator. |
