...
| UserSettingsModal | User Settings modal: fixed size (`h-[min(640px,90vh)]`) so tab changes don’t resize; GSAP open/close + **directional slide** on tab switch (`activeTab` highlights sidebar immediately; content exits horizontally → swaps `displayedTab` → enters from the opposite side; `overflow-x-hidden` clips the slide so it doesn’t flash). My Account: display name, username, avatar/banner. **Profiles:** Personal/Work identity (name/avatar/banner), active server profile, per-friend visibility (Personal / Work / Both). **Privacy & Safety:** DMs, calls, friend requests, voice-channel visibility, online status, speaking indicator. |
